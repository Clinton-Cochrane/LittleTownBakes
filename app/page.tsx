...
import MenuPreloader from "@/components/menu/MenuPreloader";
import {
	getAboutContent,
	toFacebookUrl,
	toInstagramUrl,
...
export default async function Home() {
	...
	const hasAbout = about.story || about.howWeBake || about.whoWeAre || about.values;
	const hasHowToOrder = howToOrder.intro
			...
			{hasAbout && (
				<section className="mb-10">
					...
					<div className="card-warm flex flex-col gap-4 p-5 sm:p-6">
						...
						{about.values && <p className="text-cocoa">{about.values}</p>}
					</div>
				</section>
			)}
					...
					<div className="card-warm flex flex-col gap-4 p-5 sm:p-6">
						...
						{howToOrder.steps.length > 0 && (
							<ol className="list-inside list-decimal space-y-2 text-cocoa">
								{howToOrder.steps.map((step) => <li key={step}>{step}</li>)}
							</ol>
						)}
						{(howToOrder.pickupLocation
							|| howToOrder.pickupHours
							...
							<dl className="grid gap-2 text-sm">
								{howToOrder.pickupLocation && (
									<>
										<dt className="font-medium text-caramel">Pickup location</dt>
										<dd className="text-cocoa">{howToOrder.pickupLocation}</dd>
									</>
								)}
								{howToOrder.pickupHours && (
									<>
										<dt className="font-medium text-caramel">Pickup hours</dt>
										<dd className="text-cocoa">{howToOrder.pickupHours}</dd>
									</>
								)}
								{howToOrder.leadTime && (
									<>
										<dt className="font-medium text-caramel">Lead time</dt>
										<dd className="text-cocoa">{howToOrder.leadTime}</dd>
									</>
								)}
								{howToOrder.payment && (
									<>
										<dt className="font-medium text-caramel">Payment</dt>
										<dd className="text-cocoa">{howToOrder.payment}</dd>
									</>
								)}
							</dl>
						)}
